...
선별(triage)과 문서 렌더러가 함께 쓰는 토대다.

--------------------------------------------------------------------------
[왜 글자 단위가 아니라 어절 단위인가 — 실측]

    difflib 을 글자 단위로 돌리면 한국어에서 이런 결과가 나온다:
    ...
    결과:
      - 선별: 변경 어절을 기관명 패턴과 대조할 수 없다('국가데'는 기관명이 아니다)
      - 렌더러: 대비표 강조가 단어 중간에서 끊겨 읽기 어렵다

    => 공백 기준 어절로 토큰화한 뒤 비교한다.

★ 출처: seongbeen2 브랜치(lawtrack.mas.textdiff)에서 이식(2026-07-30).

--------------------------------------------------------------------------
[summarizer.matching.describe_change() 와의 관계 — 2026-07-31 정리]
...
def tokenize(text: str) -> list[str]:
    ...
    공백을 버리면 재결합 시 원문이 복원되지 않아 대비표에 쓸 수 없다.
    """
...
def diff_segments(old: str, new: str) -> tuple[list[Segment], list[Segment]]:
    """(현행 세그먼트, 개정안 세그먼트) 를 돌려준다.

    각 리스트의 text 를 순서대로 이으면 원문이 그대로 복원된다.
    """
...
def _diff_tokens(a: list[str], b: list[str]) -> tuple[list[Segment], list[Segment]]:
    ...
    ★ 공통 접두부터 뗀다 (2026-07-31, 공공기관의 정보공개에 관한 법률
...
def _split_list_items(text: str) -> list[tuple[str, str]]:
    ...
    쌍의 리스트로 쪼갠다. 각 쌍의 두 문자열을 이어붙이면 그 항목의 원문이
    그대로 복원되고, 전체를 이어붙이면 text 전체가 복원된다."""
...
def _diff_list_items(
    ...
    """목록 항목 단위로 맞춰본다 — 구분자 차이는 비교에서 무시하고,
    안 맞는 항목끼리만 어절 단위로 재귀 비교한다."""
