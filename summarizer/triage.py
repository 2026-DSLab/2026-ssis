...
이 절반을 LLM 에 보내지 않으면 비용과 보고서 노이즈가 동시에 줌.

--------------------------------------------------------------------------
[안전 원칙 — 이 모듈에서 가장 중요한 것]

    오분류의 두 방향은 비용이 전혀 다름.

      실질 변경 → FORMAL 로 오분류  : 보고서에서 누락됨. 치명적.
      형식 정비 → SUBSTANTIVE 로 오분류 : LLM 호출 한 번 더. 사소함.

    따라서 이 계층은 확신이 있을 때만 FORMAL 을 줌.
    조금이라도 애매하면 BORDERLINE 으로 넘겨 LLM 이 판단하게 함.
    "거르는 것"이 목적이 아니라 "확실한 것만 빼는 것"이 목적임.
--------------------------------------------------------------------------

출처: seongbeen2 브랜치(lawtrack.mas.triage)에서 이식.
  ArticleAgent.run()(summarizer/agents.py)이 LLM을 부르기 전에 이 판정을
  거침 — move_is_identical/no_change와 같은 자리에 있는, 세 번째
  규칙기반 스킵 경로임.
"""
...
class ChangeClass(str, Enum):
    NO_CHANGE = "변경없음"
    """개정 전후 본문이 동일한 행. 상류 파이프라인의 산물이며 보고서에서 제외함.

    실측: 1002건 중 28건이 old_text == new_text 였고 26건은 match_status='성공'
    이었음. 대부분 행정규칙(조달청 내자구매업무 처리규정 10, 계약예규 15).
    신구조문대비표에서 변경되지 않은 항목까지 행으로 뜬 것으로 보임.
    상류를 고치기 전까지 여기서 막음."""

    FORMAL = "형식정비"
    """기관명·인용법령명 정비 등 실무 영향이 없는 변경. 부록으로 보냄."""

    SUBSTANTIVE = "실질변경"
    """내용이 바뀐 변경. 요약 대상."""

    BORDERLINE = "판단필요"
    """결정론적으로 확신할 수 없음. LLM 분류 에이전트로 보냄."""
...
#     기관명 정비로 분류됐음. 금액 변경이 보고서에서 누락될 뻔했음.
#     '국'(전국·외국), '과'(결과·경과), '실'(사실·현실) 도 같은 문제임.
#
#     => 한 글자 접미사 중 모호한 것은 전부 뺌.
#        '원'이 필요한 기관은 진흥원·연구원처럼 통째로 적음.
#        접미사 앞에 최소 2음절을 요구해 '일부를'·'전부를' 같은 일반어를 막음.
#        숫자가 섞인 어절은 무조건 제외함 — 수치 변경은 실질 변경임.
...
# 이 임계값들은 위 실측 분포에서 나왔음. 보수적으로 잡혀 있음.
_FORMAL_MAX_WORDS = 4
"""변경 어절이 이보다 많으면 단순 명칭 정비로 보기 어려움.

FORMAL 판정의 실질적인 근거는 유사도가 아니라 '변경 어절이 전부
기관·직위명인가' 임. 유사도를 함께 요구했더니 짧은 조문에서
어절 하나만 바뀌어도(예: 5어절 중 1개 → 유사도 0.80) 문턱에 걸려
LLM 으로 밀려났음. 기관명 판정이 이미 충분히 좁으므로 유사도는
아래 _FORMAL_MIN_SIM 을 하한선으로만 씀."""

_FORMAL_MIN_SIM = 0.50
"""SequenceMatcher 가 이상 정렬을 낸 경우를 막는 하한선.
정상적인 명칭 정비라면 이 값을 밑돌 수 없음."""

_SUBSTANTIVE_MAX_SIM = 0.80
"""이 아래면 결정론적으로 실질 변경 확정 — 단, 기관명 정비는 예외로
먼저 검사함(짧은 조문은 명칭 하나만 바뀌어도 이 값을 밑돎)."""
...
def _is_org_rename(dels: list[str], inss: list[str]) -> bool:
    ...
    [실패 사례 — 실데이터에서 잡힌 것]
        '심의위원회가' -> '보건복지부장관이'
        양쪽 다 기관명 패턴에 맞아 형식 정비로 분류됐음. 그러나 이것은
        개편이 아니라 권한 이관임 — 심의 권한이 위원회에서 장관으로
        넘어간 실질 변경이며, 보고서에서 누락되면 안 됨.

        => 기관(body)과 기관장 직위(title)는 서로 다른 범주로 보고,
           범주가 바뀌면 명칭 정비로 인정하지 않음.
           '통계청장'->'국가데이터처장'(title→title)은 통과하고
           '심의위원회'->'보건복지부장관'(body→title)은 걸림.
    """
...
def _is_numeric_change(dels: list[str], inss: list[str]) -> bool:
    ...
    [이 검사가 필요한 이유 — 테스트가 잡은 위험]
        ...
        BORDERLINE(판단 유보)으로 빠짐. 위임 규칙에서는 LLM 이
        형식정비라고 답하면 그대로 형식정비가 되어 금액 변경이
        보고서에서 누락됨.

        수치 변경은 실무 영향이 사실상 확실하므로 코드가 확정 판단해
        위임 구간에서 빼냄. 단, '제47조' -> '제47조제2항' 같은
        조문 인용은 숫자가 있어도 수치 변경이 아님.
    """
...
def triage(old_text: str, new_text: str, *, change_type: str = "") -> TriageResult:
    ...
    신설·삭제는 내용 자체가 생기거나 사라진 것이므로 형식 정비일 수 없음.
    """
    ...
    # 명칭 정비 검사가 먼저임. 짧은 조문은 명칭 하나만 바뀌어도
    # 유사도가 실질 변경 구간까지 떨어지기 때문임.
    ...
    # 수치 변경은 유사도와 무관하게 확정 판단함. BORDERLINE 으로 흘려보내면
    # LLM 이 형식정비라 답했을 때 금액·기한 변경이 그대로 누락됨.
