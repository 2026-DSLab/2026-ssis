"""HWPX 보고서 검증 — 생성된 문서를 다시 읽어 원본과 대조한다.

★ 왜 필요한가:
    HWPX 는 XML+ZIP 이라, 요약 텍스트를 넣을 때 특수문자(&, <, ①, ㆍ)나
    긴 문단에서 글자가 빠지거나 태그 조각이 새어들 수 있다. 요약 검증
    (요약이 원문과 맞나)과 별개로, '요약이 문서에 온전히 들어갔나'를
    확인한다.

    방법은 요약 검증과 같다 — LLM 없이 코드 대조. 생성한 HWPX 를 다시 열어
    본문 텍스트를 뽑고, 보고서에 실었어야 할 원본 텍스트가 다 들어갔는지
    본다. 빠진 조각이 있으면 어느 것인지 보고한다.
"""
...
def _expected_fragments(contract: ContractSummary) -> list[str]:
    ...
    새 보고서는 overview(취지)와 조문 요약(표)을 싣는다. 그 '내용'이 문서에
    온전히 들어갔는지 본다. 장식(─, ■ 기호)은 검사 대상이 아니다.
    """
    frags: list[str] = []
...
def verify_report(contract: ContractSummary, hwpx_path: str | Path) -> list[str]:
    """생성된 HWPX 를 원본과 대조한다. 빠진 조각 목록을 돌려준다(없으면 빈 목록)."""
