"""결정론적 본문 조립 — 구조 사실은 코드가 씀.

왜 필요한가 (실측, 범죄피해자 제47조):
    LawAgent(LLM)에게 body 를 자유롭게 쓰게 했더니 이동 항목을 "신설"로
    뭉뚱그렸음. 매핑은 "구②→④ 이동"을 정확히 잡았는데도, LLM 이 산문으로
    풀어 쓰는 과정에서 "제47조가 신설되었습니다"라고 틀리게 서술했음.

    구조 사실(어느 조가 신설/개정/이동/삭제인가)을 LLM 판단에 맡기면
    모델 크기와 무관하게 확률적으로 틀림. 그래서 그 부분은 코드가 씀 —
    매핑(12개 그룹 검증 완료)과 조문 요약을 그대로 박으므로 절대 안 틀림.
    LLM 은 '개정 취지 개요'만 씀(그건 revision_reason 을 다듬는 일이라
    구조를 헷갈릴 여지가 없음).
"""
...
def build_change_section(summaries: Sequence[ArticleSummary]) -> str:
    """조문별 변경 내용을 결정론적으로 조립함.

    각 항목의 change_type 은 apply_mappings 가 교정한 값이라 정확함.
    이동 항목은 조문 요약에 이미 "○에서 ○로 번호만 이동" 문장이 들어 있음.
    """
    if not summaries:
        return ""
    ...
    for art, items in by_art.items():
        ...
        # 다 펼치지 않음 — 수십 줄이 되어 담당자가 읽기 어려움. "조문 전체
        # 신설"로 묶고, 대표 내용(첫 항목) 한 줄만 보임. 세부는 원문 참조.
