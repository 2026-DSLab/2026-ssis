"""요약문 후처리 — LLM 이 낸 표면적 오류를 결정론적으로 바로잡는다.

프롬프트로 "한자 쓰지 마"라고 부탁하는 것과 별개로, 출력을 코드로 한 번
더 훑는다. LLM 은 확률적이라 프롬프트만으로는 100% 못 막는다. 모델을
바꿔도 이 계층은 그대로 안전망으로 남는다.
"""
...
# 표기다. 예: "벌금형을 과(科)한다", "오륜(五輪)". 이건 건드리지 않는다.
_PARENTHETICAL = re.compile(r"[가-힣]\s*[(（]\s*[一-鿿㐀-䶿]+\s*[)）]")

# LLM 이 한글 단어를 쓰다 첫 글자를 한자로 잘못 낸 흔한 경우.
# 실측(2026-07-23, 범죄피해자 요약): "具체적" → "구체적".
# 병기가 아닌 단독 한자만 대상이며, 애매하면 치환하지 않고 경고로 남긴다.
_COMMON_FIX = {
    "具체적": "구체적",
    "科학": "과학",
    "課": "과",
...
def _mask_parentheticals(text: str) -> tuple[str, list[str]]:
    """병기 한자를 임시 치환해 보호한다. (보호된 텍스트, 원본조각들)"""
    saved: list[str] = []
...
def fix_text(text: str) -> tuple[str, list[str]]:
    """한 문자열을 후처리한다. (고친 텍스트, 남은 경고들) 반환.

    - 병기 한자(과(科) 등)는 보존.
    - 흔한 한자 오타는 한글로 치환.
    - 그래도 남은 단독 한자는 경고로 보고(치환하지 않는다 — 잘못 바꾸느니
      사람이 확인하게 둔다).
    """
    if not text:
        return text, []
...
def clean_summary(headline: str, body: str) -> tuple[str, str, list[str]]:
    """headline/body 를 후처리하고 남은 경고를 모은다."""
