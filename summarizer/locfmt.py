...
★ 왜 여기 하나로 모으는가(2026-08-03): 이 규칙은 원래 report/builder.py
(HWPX 표의 "조문"/"위치" 칸)에만 있었는데, agents.py의 "번호만 이동"
문장("②5.에서 제8조②12.로 이동했습니다")도 LLM을 부르지 않는 규칙 기반
문장이라 똑같은 위치 표기 규칙이 그대로 필요해졌다(사용자 리포트:
"②5. 이렇게 말고 2항 5호처럼 쓰기 힘든 상태야?"). 완전히 같은 로직을
agents.py에 또 복사하는 대신, summarizer 패키지 안에서 공유한다 — 두
호출부 다 무거운 의존성(hwpx 등)이 필요 없는 순수 문자열 함수라 공유해도
비용이 없다. (webapp/app.py는 별도로 자기 사본을 유지한다 — 그쪽은
HTML span으로 감싸야 해서 반환 타입 자체가 다르고, 무거운 hwpx 패키지를
끌어오면 안 되는 별도 이유가 있다: report/builder.py 상단 주석 참고.)
"""
...
def format_position(rest: str) -> str:
    """위치 문자열(예: '⑥1.가.')을 사람이 읽는 '⑥항·1호·가목'으로 바꾼다.

    숫자 뒤에 "항"/"호"/"목" 글자를 붙이고, 조각 사이는 가운뎃점(·)으로
    구분한다(원래 표기의 마침표는 한글 단위로 대체되므로 뗀다).
    """
    if not rest:
        return rest
    out: list[str] = []
