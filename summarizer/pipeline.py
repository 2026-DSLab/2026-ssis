...
class SummaryPipeline:
    def __init__(self, client: LLMClient, settings: Settings):
        ...
        # 검증은 LLM 이 아니라 코드(verify_summaries)가 함.
    ...
    def run_law(self, law: LawChange) -> LawSummary:
        ...
        # 교정을 1단계 '앞'에서 함. 뒤에서 하면 1단계 요약은 계약의
        #   틀린 라벨대로 만들어지고, 2단계가 서로 모순된 정보를 받음.
        #   실측: 그 상태에서 모델이 틀린 쪽을 택해 "①8이
        #   신설되었다"는 잘못된 요약이 나왔음.
        ...
        if not units:
            # 조문 변경이 없어도 2단계는 부름 — 공식 개정이유만으로도
            # 요약할 내용이 있고, 빈 결과를 내보내는 것보다 나음.
        ...
        # 잡음. LLM 판단을 쓰지 않으므로 오탐(취향·분량 트집)이 없음.
        #
        # 설계: 감수에서 걸린 내용을
        # caveats 문장으로 만들어 내보내던 걸 그만둠 — caveats_for()를
        # 완전히 제거했던(webapp "확인이 필요한 항목" 박스 삭제) 것과
        # 같은 이유임: 검증 결과가 정답이라 해도, 이걸 "※ 확인 필요"
        # 형태로 사용자에게 노출하면 그 자체가 불안감을 조성한다는 게
        # 이번 세션 내내 반복된 결론이었음. verifier_issues 는 계속
        # 저장함(DB/내부 점검용 원자료로서의 가치는 남아 있음) — 다만
    ...
    def run_file(self, path: str | Path) -> ContractSummary:
        """계약 파일 1개 — single 이든 weekly 든 동일하게 처리함."""
        path = Path(path)
        ...
        return ContractSummary(
            ...
            laws=summaries,
            # unresolved / no_comparison 은 요약하지 않고 그대로 통과시킴.
            # 위치 확정에 실패한 것을 문장으로 만들면 그게 환각임.
