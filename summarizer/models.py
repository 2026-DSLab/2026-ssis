...
파이프라인 '내부' 및 '출력' 스키마다. 둘을 섞지 않는다 — 계약은 우리가
바꿀 수 없는 남의 것이고, 이건 우리가 자유롭게 바꿀 수 있는 우리 것이다.
"""
...
class ArticleUnit:
    ...
    개정 전/후 문장"으로 환원된다. 여기서 그 차이를 흡수해 1단계
    에이전트가 배열 종류를 신경 쓰지 않게 한다.

    ★ old_text_is_context 가 이 구조의 핵심이다.
      True 면 old_text 는 "이 위치의 개정 전 문장"이 아니라 참고 맥락일
      뿐이다. 프롬프트에서 반드시 그렇게 표시해야 한다 — 빠뜨리면 LLM 이
      서로 대응하지 않는 두 문장을 나란히 놓고 "이렇게 바뀌었다"고
      환각한다. 계약의 match_status 주석이 경고하는 바로 그 상황이다.
    """
    ...
    change_type 이 '이동' / '이동후개정' 일 때만 채워진다. 매핑 단계가
    계약의 틀린 라벨을 교정하면서 넣는다."""
...
class ArticleMapping:
    ...
    """LLM 이 판정했고 확신이 낮은 경우. 보고서에 원문 링크를 함께 낸다."""
...
class ArticleSummary:
    ...
    2단계로 그대로 전달해 종합 요약에서도 단정하지 않게 한다."""

    error: str | None = None
    """호출 실패 시 사유. 실패를 조용히 빼지 않는다는 계약 원칙과 같다."""
...
class LawSummary:
    ...
    ★ 요약을 DB 에 적재할 때(sinks.DbSink) (law_id, new_serial_no) 가
      키가 된다. law_id 만으로는 부족하다 — 같은 법이 여러 번 개정되면
      요약도 여러 건이고, 그것들은 서로 다른 사실을 말한다."""
...
class ContractSummary:
    ...
    single 이든 weekly 든 구조는 같다 — single 은 laws 가 1건일 뿐이다.
    """
    ...
    """6가드로 위치 확정에 실패한 건. 요약하지 않고 그대로 통과시킨다 —
    확정 안 된 것을 문장으로 만들면 그게 환각이다."""
