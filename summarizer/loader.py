...
#: matching.py resolve_article()가 이미 실측(2026-07-22, 전자정부법
#: 제56조의2)으로 이 정확한 케이스를 분석해뒀다 — 완전일치는 코드가
#: 못 풀고(문장이 확장됨), LLM 판정으로 넘어간 것 자체가 이미 "애매함"의
#: 신호다. 그런데 LLM이 "이동"이라 답하면 이 함수가 그걸 그대로 믿고
#: move_is_identical=True 를 줘서 검증·요약을 통째로 건너뛰고 있었다.
#: 실측(2026-07-30): 그 LLM 판정 자체가 note 에는 "재구성됨"이라 써놓고도
#: relation 은 "이동"을 골라 자기모순인 응답을 낸 사례가 실제로 나왔다
#: (해당 쌍의 content_ratio=0.80 — 진짜 동일 이동 사례들은 전부 1.0에
#: 가깝다). LLM의 "이동" 주장을 곧이곧대로 믿지 않고, 여기서 코드로
#: 한 번 더 확인한다 — 이 프로젝트 전체의 원칙("LLM 자기 판단을 검증에
#: 쓰지 않는다")과 같은 이유다.
...
#: ★★ 실측(2026-07-31, 전자정부법 제2조11. "정보자원" 정의): 개정 전
...
#: 이었다 — 이런 경우는 매칭하면 안 된다. "부분문자열이면 확정"이라는
#: 규칙 하나로는, 아주 짧은 조각(예: "정보")이 우연히 여기저기 걸릴
#: 오탐 위험이 있어 최소 길이를 둔다. 4는 실측에서 나온 가장 짧은 진짜
#: 매치("행정정보")의 길이다 — 이보다 짧은 매치는 오탐 위험이 더 크다고
#: 보고 확정하지 않는다(대신 기존처럼 "구조확장" 미확정으로 남긴다).
_MIN_EXPANSION_MATCH_LEN = 4


def _normalize_with_map(text: str) -> tuple[str, list[int]]:
    """normalize()와 같은 규칙(앞 번호 제거 + 공백 제거)으로 정규화하되,
    정규화된 문자열의 각 글자가 원문의 몇 번째 인덱스였는지도 같이 돌려준다.

    매치를 찾은 뒤 원문 그대로(공백ㆍ서식 보존)의 조각을 잘라내려면 이
    역매핑이 필요하다 — normalize() 단독으로는 매치 위치를 원문으로
    되돌릴 수 없다.
    """
    text = text or ""
...
def _find_content_match(
    ...
    그대로의 해당 조각을 잘라 돌려준다. 없으면 None.

    consumed: old_text 정규화 문자열과 길이가 같은 리스트. 이미 다른
    항목이 근거로 쓴 구간은 True로 표시해, 서로 다른 두 새 항목이 같은
    옛 조각 하나를 중복해서 근거로 삼지 않게 한다(실측 사례엔 없었지만,
    짧은 단어가 반복되는 조문에서 안전장치로 필요하다).
    """
...
def _units_from_expansion(law: LawChange, group: StructuralExpansion) -> list[ArticleUnit]:
    ...
    ★ 2026-07-31 개선: 예전엔 모든 유닛을 "구조확장(구법미분리)"으로만
    표시했다(옛 프로즈 문장이 항목별로 안 나뉘어 있어 어차피 대응을
    확정 못 한다는 전제). 그런데 실측해보니 그 전제가 항상 참은 아니었다
    — 프로즈가 쉼표로 나열된 목록이었고, 그 목록 문구가 새 항목 문구
    안에 그대로 남아 있는 경우가 있었다(위 _MIN_EXPANSION_MATCH_LEN
    주석 참고). 이런 항목은 old_text 전체가 아니라 실제로 대응하는
    조각만 잘라 match_status="성공"으로 확정한다 — 나머지(진짜 새 내용)
    만 예전처럼 미확정으로 남긴다.
    """
...
def apply_mappings(
    ...
    for unit in units:
        ...
        if m.relation == "신설":
            ...
            # "LLM판정"인 경우 그 주장 자체가 틀릴 수 있다(위 상수 설명
            # 참고). 코드로 실제 유사도를 한 번 더 확인한 뒤에만 믿는다.
...
def caveats_for(unit: ArticleUnit) -> list[str]:
    ...
    ★★★ 설계(2026-07-31, 사용자 결정): match_status 기반 캐비어트를
    전부 없앴다. 2026-07-31 하루 동안 이 함수를 세 번 좁혔다 —
