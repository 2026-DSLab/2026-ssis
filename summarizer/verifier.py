...
def _numbers_in(text: str) -> set[str]:
    """텍스트에 나오는 숫자만 정규화(쉼표 제거)해서 뽑는다.

    ★★ 실측(2026-07-31, 영유아보육법 제8조②9.): "9. 영유아 보육ㆍ교육..."
    처럼 항목 번호 뒤의 "."은 소수점이 아니라 목록 구분자인데, 정규식이
    "9."까지 하나의 숫자로 통째로 잡았다. 그런데 요약 문장 "9번 항목으로
    옮겨왔습니다"에서는 "9" 뒤에 "번"이 와서 "9"만 잡힌다 — "9." ≠ "9"라
    같은 숫자인데도 다른 문자열로 보여 없는 숫자로 오탐(환각 아님)했다.
    뒤에 숫자가 안 이어지는 "."은 소수점이 아니므로 떼어낸다("9.5"처럼
    뒤에 숫자가 있으면 진짜 소수점이라 그대로 둔다 — \\d* 매치라 이미
    "9.5"를 통째로 잡으므로 이 케이스는 애초에 끝이 "."로 안 끝난다).
    """
    out: set[str] = set()
...
def _check_article(s: ArticleSummary) -> list[VerifierIssue]:
    ...
    #    검증한다. ①의 인용부호 검사는 요약이 스스로 따옴표를 친 곳만
    #    보므로, "구체적으로/자연스럽게 쓰라"는 프롬프트를 따라 LLM이
    #    따옴표 없이 통짜 문장을 쓰면(권장되는 방식이다) 그 안의 숫자는
    #    전혀 검증되지 않는 사각지대가 있었다.
    #
    #    ★★ 실측(2026-07-31, 국가를 당사자로 하는 계약에 관한 법률
    #    제28조②): describe_change()가 "20일"→"30일"을 "'2'→'3'"으로
    #    쪼갠 버그 때문에 LLM이 "당사자의 수가 '2'에서 '3'으로 변경"이라는
    #    완전히 없는 내용을 지어냈다. 그 버그는 diff 단계에서 고쳤지만
    #    (숫자가 항상 단위와 함께 나오게), 이 검사는 그 근본 버그와
    #    무관하게 "요약에 등장한 숫자가 원문에 아예 없으면 위험 신호"라는
    #    일반 규칙이라 앞으로 비슷한 사고(다른 원인이더라도)를 잡아낸다.
    #    숫자는 문장처럼 의역되지 않는다 — 원문에 없는 숫자가 요약에
    #    있으면 예외 없이 지어낸 것이다(오탐 위험이 없는 이유).
