...
왜 요약이 기본값이 아닌가:
    1~5 단계는 공짜지만 6단계부터는 호출 건당 LLM 비용이 듦. 기본을
    켜 두면 "감지 결과만 보려고" 돌린 실행에서도 조용히 과금됨.
    자동 실행(작업 스케줄러)에는 --full 로 등록함 — scripts/weekly.cmd
    가 그렇게 되어 있음.

이 스크립트가 하지 않는 것 (detect.py 상단 docstring과 동일한 경계):
    병렬 처리, 재시도 정책. 스케줄 등록 자체는 scripts/register_task.ps1
    이 맡음 — 여기서는 "한 번 실행하면 전체가 정확히 처리된다"는 것만
    보장함.

한 항목에서 API 오류/예외가 나도 전체 배치를 중단하지 않고 나머지를
계속 처리함 — 워치리스트 100건 중 1건이 실패했다고 나머지 99건의
개정 감지 기회를 날리면 안 되기 때문임. 실패한 항목은 요약에 모아
보고하고, 종료 코드로 오류 발생 여부를 알림(오류 0건이면 0, 있으면 1
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    # --hwpx / --summary-db 는 요약 결과가 있어야 의미가 있음. 사용자가
    # --summarize 를 빠뜨렸다고 "아무것도 안 나오는" 실행을 하게 두지 않음.
    if args.full:
...
def run_summary_stage(
    ...
    """계약 JSON 하나를 요약 파이프라인에 넘김. 오류 건수를 돌려줌.

    import 를 함수 안에서 하는 이유: 요약 단계는 LLM SDK 와 hwpx 를
      필요로 하는데, 감지만 쓰는 사람은 그것들을 깔지 않았을 수 있음.
      모듈 최상단에서 import 하면 --summarize 를 안 쓴 실행도 함께
      죽음.

    여기서 예외를 잡아 삼키지 않고 건수로 돌려주는 이유: 감지 결과는
      이미 DB 에 커밋되어 안전함. 요약이 실패했다고 그 사실을 되돌릴
      수는 없으므로, 배치는 계속 진행하되 종료 코드로 알림.
    """
    print("\n" + "=" * 70)
    print("요약 단계 시작 (LLM)")
    print("=" * 70)
