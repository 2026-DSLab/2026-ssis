...
| `batch_date` / `source_file` | | 어느 배치의 어느 계약 JSON에서 나왔는지(재현·추적용). `batch_date`가 NULL이면 주간 배치가 아니라 기간별 즉석 조회가 만든 요약. 화면 선별에는 안 쓴다 — 주간 리포트는 `enforce_date`로 자른다 |
...
| `GET /summary` | 지난 달력 주(월~일, `lawtrack.week.last_full_week()`)에 **시행된** 법령별 요약을 렌더링(`law_summary.enforce_date` 기준). `?period=5d\|2w\|1m` 쿼리파라미터가 있으면 기간별 즉석 조회 결과로 대체 |
| `GET /download` | 같은 창(또는 period)의 요약 행들로 HWPX 보고서를 그 자리에서 조립해 내려줌 — LLM 없이 DB→파일 변환이라, 화면 목록과 파일 내용이 항상 같음 |
...
다시 훑어 실제로 감지를 재실행한다(주간 리포트 탭과 다름 — 그건 DB만 읽는다).
이미 요약된 개정분은 LLM을 다시 부르지 않고 DB에서 그대로 재사용하며(비용
낭비 방지), 새로 감지된 것만 그 자리에서 요약한다.

네 탭 모두 `law_summary.enforce_date`(시행일)로 창을 자른다
(`LawSummaryRepo.fetch_by_enforce_period`). 다른 건 창의 길이와, 그 목록을
만들기 전에 실 API 스윕을 도는지 여부뿐이다. 창이 서로 어긋나 있어
"주간 리포트"보다 "최근 5일"이 많을 수 있다.

| 탭 | 창 | 계산 시점 |
|---|---|---|
| 주간 리포트 | 지난 달력 주 월~일(고정) | DB 조회만 |
| 최근 N일 | 오늘부터 N일 전(매번 이동) | 클릭 시점에 실 API 스윕 후 DB 조회 |

주간 리포트의 창은 지난 한 주라, 이번 주 들어 시행된 개정은 여기 안 뜨고
"최근 5일"에만 뜬다. `webapp/app.py`의 `_after_week_notice()`가
그 차이를 화면에서 메운다 — 창 다음 날부터 오늘까지 건수를 세어(목록이
아니라 `count_by_enforce_period`) 0보다 크면 안내 줄을 낸다. 링크는 그
건들을 **전부** 담는 가장 짧은 프리셋으로 보낸다(일요일에 열면 알릴
구간이 6일이라 "최근 5일"이 아니라 "최근 2주"). "N건 더 있다"고 해놓고
눌렀더니 다 안 보이면 같은 종류의 어긋남을 새로 만드는 셈이다.

**감지일(`created_at`)로 창을 자르면 안 된다.** 한때 "최근 N일"이 그렇게
했는데, `_run_live_sweep()` 한 함수 안에서 요약 대상은 시행일로 고르고
화면 목록은 감지일로 골라 기준이 갈렸다. 그래서 시행이 창 밖인 개정이
딸려 들어왔다 — 실측: 최근 5일 탭에 시행일 7일 전(개인정보 보호법 시행령),
최근 1개월 탭에 시행일 217일 전(지능정보화 기본법). 감지일은 우리가 언제
돌렸는지에 달린 값이라 기간의 뜻을 담지 못한다. `created_at` 컬럼 자체는
"언제 처음 봤나" 기록으로 계속 쓰되, 창을 자르는 데는 쓰지 않는다.

주간 리포트가 배치가 아니라 달력 주를 창으로 쓰는 이유는
`src/lawtrack/week.py` 의 모듈 독스트링에 있다 — 요약하면, 배치 실행
시각에 창이 끌려다니면 같은 주 안에서도 볼 때마다 건수가 달라졌다.
...
**`.env`가 없는 상태에서도 480건 전부 통과해야 정상이다**(실측 확인). 실패가
...
python -m pytest -q         # 480 passed 확인
