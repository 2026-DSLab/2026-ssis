...
run_weekly.py와 로직은 같음(watchlist 전체 순회 → process_entry() →
build_contract() → 필요하면 LLM 요약). 다른 점은 실행 주체(작업
스케줄러가 아니라 웹 요청)와 대상 기간(고정 7일이 아니라 프리셋
5일/2주/1개월)뿐임.

비용/시간: watchlist 전체(현재 102건)에 실제 API를 순차 호출함.
대부분은 이미 최신 버전이 저장돼 있어 즉시 "변경없음"으로 끝나지만
(VersionRepo.law_exists가 이미 True라 API 호출 자체는 피할 수 없음 —
"바뀌었는지"를 확인하려면 매번 최신 버전을 조회해야 함), 개정이 실제로
발견된 항목만 LLM 요약까지 추가로 돎. 그래서 요청마다 걸리는 시간이
다름 — 새 개정이 없으면 API 호출 시간만(수십 초), 있으면 + LLM 요약
시간이 더해짐("대기가 있더라도 괜찮다"를 전제로 함).

캐싱: 같은 기간(window_key)을 짧은 시간 안에 다시 요청하면(새로고침 등)
매번 API 102번 + LLM을 다시 부르지 않도록 TTL 캐시를 둠.
"""
...
#: 자유 날짜 선택 대신 프리셋으로 고정한 이유:
#: 요청마다 실 API+LLM 비용이 드는 기능이라, 기간을 몇 개로 한정해야
#: 캐시가 의미 있게 재사용됨(자유 날짜면 사람마다 범위가 미묘하게
#: 달라 캐시 적중률이 거의 0에 수렴함).
...
#: 등). 웹페이지 로딩 화면이 폴링해서 보여줌 — 지금 어느 단계인지
#: 담당자가 알 수 있어야 한다는 요구에서 나왔음.
...
def get_period_result(settings: Settings, window_key: str, *, force: bool = False) -> PeriodResult:
    """캐시에 있고 안 만료됐으면 재사용, 아니면 실제로 감지 스윕을 돎.

    (동기 버전 — 백그라운드 미리캐싱 루프가 씀. 웹 요청 경로는 폴링
    가능한 ensure_sweep_started()/get_progress()를 대신 씀.)
    """
...
def ensure_sweep_started(settings: Settings, window_key: str) -> bool:
    ...
    가도 됨)를 돌려줌. 아니면 백그라운드 스레드로 실 스윕을 시작하고
    True(= /period-status를 폴링하며 기다려야 함)를 돌려줌. 이미 같은
    기간의 스윕이 돌고 있으면 새로 시작하지 않고 그냥 True만 돌려줌
...
#: 그대로 더해져 90초~4분씩 걸렸음 — 캐싱/LLM 스킵은 "얼마나 자주/헛되이
#: 도느냐"만 줄일 뿐 이 스윕 자체의 소요시간은 못 줄임.
#: 실측(실 국가법령정보 API 대상): 5=17.5초, 10=10.2초,
#: 20=5.8초, 30=4.4초 — 전 구간 에러 0건. 20을 넘어가면 수익이 급격히
#: 줄어듦(가장 느린 개별 요청 하나가 병목이 되는 지점) — 정부 서버에
#: 필요 이상의 동시 요청을 보내지 않으면서도 체감 속도(90초→6초대,
#: 약 15배)를 얻을 수 있는 지점으로 20을 골랐음.
...
def _process_one_entry(
    ...
    LawApiClient를 쓰므로(공유 안 함) 스레드 간 상태 충돌이 없음."""
...
def _run_live_sweep(settings: Settings, window_key: str) -> PeriodResult:
    ...
    # 없음 — 커넥션을 오래 붙들고 있는 경우를 대비해 조금 더 넉넉히 둠.
    ...
    # 다시 잡힘 — 5분/10분마다 미리 캐싱을 돌리면 이미 요약해 둔 같은
    # 개정 건을 계속 다시 LLM에 넘기게 됨(30일 창이면 한 건이 최대
    # 30일 동안 매번 재요약될 수 있음 — 완전히 낭비). law_summary에
    # (law_id, new_serial_no)가 이미 있으면 그 요약은 "완결된 사실"이라
    # 다시 부를 이유가 없음 — 진짜 처음 보는 것만 골라 LLM을 부름.
    unsummarized_keys = {
    ...
    if unsummarized_keys:
        ...
        # 실측 발견: write_contract()는 파일명을 항상
        # "weekly_contract_{batch_date}.json"로 고정함. 기간(window_key)이
        # 달라도 batch_date(오늘 날짜)는 같으므로, 같은 날 "5일"과 "2주"가
        # 둘 다 새 개정을 찾으면 서로 같은 파일명을 놓고 경쟁해 나중 것이
        # 먼저 것을 덮어씀 — law_summary.source_file은 "5일" 결과를
        # 가리키는데 실제 파일 내용은 "2주" 것으로 바뀌는 불일치가 생김.
        # 파일명에 window_key를 넣어 완전히 분리함(보고서도 최근 5일과
        # 최근 2주가 각각 다르게 나와야 한다는 요구).
...
def _filter_contract(contract: WeeklyContract, keys: set[tuple[str, str]]) -> WeeklyContract:
    """LLM 요약 대상을 unsummarized_keys로만 좁힌 계약 사본을 만듦.

    unresolved/no_comparison은 애초에 LLM이 손대지 않는 항목이라 그대로
    들고 감 — 걸러야 할 건 amendment_groups(=LLM이 실제로 부를 대상)뿐.
    """
    filtered_groups = []
    for group in contract.amendment_groups:
...
def _row_to_law_summary(row: dict):
    ...
    DbSink가 쓰는 방향(LawSummary → asdict → JSON → DB)의 정반대임.
    HWPX 재구성은 이미 완결된 요약을 다시 파일로 옮기는 것뿐이라 LLM을
    부르지 않음 — 이 함수가 그 "다시 조립"을 담당함.
    """
...
def start_background_refresh(
    ...
    유지함 — 방문자가 버튼을 눌렀을 때 실 스윕을 기다리지 않게 함.

    비용: LLM은 위에서 이미 "처음 보는 개정만" 부르도록 걸러뒀으므로,
    사람이 안 봐도 계속 도는 이 백그라운드 루프가 LLM 비용을 반복
    발생시키지는 않음 — 늘어나는 건 국가법령정보 API 존재확인 호출
    (10분마다 102건, 무료)뿐임.

    daemon=True: 메인 프로세스가 죽으면 이 스레드도 함께 정리됨 —
    실 서버 운영에는 별도 종료 처리가 필요 없음. stop_event는 테스트가
