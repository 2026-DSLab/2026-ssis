...
규칙이 어디에 몇 번 인용되는지 찾아(doc_match.match) 그대로 보여줌.
전 과정이 요청 처리 중 메모리에서 끝남 — DB에 아무것도 저장하지
않고, 외부 API/LLM도 부르지 않음(결정론적 문자열 매칭뿐).

매칭 사전은 database/seed_watchlist.sql 을 파싱해 만듦(doc_match
PoC와 동일). watchlist 테이블 SELECT 로 바꿀 수 있는 동일 인터페이스
지만, seed 기반을 유지하면 DB 없이도 이 페이지가 동작하고 PoC 실측
결과와 1:1 대조 검증이 가능해 일단 그대로 둠.
"""
...
#: 허용 확장자 — doc_match.extract 가 지원하는 형식과 정확히 같음.
ALLOWED_SUFFIXES = {".pdf", ".hwpx"}

#: 업로드 상한(50MB). 실측 예시 문서 4개가 2.8~10MB 라 여유 5배로 잡음.
MAX_UPLOAD_BYTES = 50 * 1024 * 1024

#: 추출 텍스트가 이보다 적으면 "스캔본(이미지) PDF" 경고를 띄움
...
#: 시행 예정)일 때만 배지를 붙임. 실측(watchlist 102건):
#: 90일 창이면 23건에만 붙어 "전부 배지"가 되는 노이즈를 피할 수 있음.
RECENT_REVISION_DAYS = 90


def fetch_revision_info(law_ids: list[str]) -> dict[str, dict]:
    """법령들의 최신 개정 정보 + (있으면) 요약 한 줄을 DB에서 가져옴.
    ...
    하나만 고름 — change_log 에 같은 개정이 중복 기록돼 있어도(실측:
    최초 채움 날 백그라운드 스윕 경쟁으로 97쌍 중복 발생) 안전함.
    """
    if not law_ids:
        return {}
    try:
...
def register_pdf_routes(
    ...
    """라우트 2개(GET/POST /pdf)를 등록함.

    사전은 첫 요청에서 한 번만 만들어 재사용함(지연 생성) —
    ...
    # 메모리가 터질 수 있어 여기서 걸어 둠(초과 시 413).
    # setdefault 를 쓰면 안 됨 — Flask 기본 설정에 이 키가 이미
    #   None 으로 존재해서 setdefault 가 조용히 아무것도 안 함(실측).
    ...
    def _upload_page(error: str | None = None, status: int = 200, doc_date: str = ""):
        ...
        # 골라 둔 날짜까지 날아가면 처음부터 다시 입력해야 함.
        # today 는 날짜 칸의 max — 브라우저가 미래 날짜를 아예 못 고르게 함
        # (서버에서도 한 번 더 막지만, 고르기 전에 막는 편이 친절함).
    ...
    def pdf_check() -> tuple[str, int]:
        ...
        #
        # 실측 버그(Windows 11): 원래 NamedTemporaryFile 을
        #   연 채로 그 이름에 file.save(tmp.name) 을 했는데, Windows 는
        #   NamedTemporaryFile 을 O_TEMPORARY 로 열어 핸들이 살아있는 동안
        #   같은 경로를 다시 열 수 없음 — 업로드마다 PermissionError 로
        #   500 이 났음(POSIX 에서는 재오픈이 되므로 macOS/Linux 에서는
        #   증상이 안 나타남). 디렉터리만 임시로 잡고 그 안에
        #   직접 파일을 만들어, 열려 있는 핸들과 경로 재오픈이 겹치지
        #   않게 함. TemporaryDirectory 는 블록을 벗어날 때 통째로
        #   지우므로 정리 보장은 그대로임.
        ...
        # 아니라 소음이 됨. 요약 한 줄은 시기 무관하게 있으면 보여줌.
        try:
        ...
        def _change_status(enforce: date | None) -> str:
            ...
            여기서 쓰는 enforce 는 "지금 시행 중인 버전의 시행일"임
            (fetch_revision_info 가 change_log 에서 법마다 가장 최근 감지분
            하나를 뽑아 줌). 그래서 이력 전체를 뒤지지 않아도 답이 나옴:

              - 시행일 > 기준일  : 지금 버전이 문서를 쓴 뒤에 시행됐음
                                   => 그 사이에 적어도 한 번 바뀌었음.
              - 시행일 <= 기준일 : 문서를 쓸 때 이미 지금 버전이 시행 중이었고
                                   그 뒤로 바뀐 게 없음(바뀌었다면 더 나중
                                   시행일을 가진 새 버전이 현행이었을 것)
                                   => 확실히 안 바뀌었음.

            실측으로 확인하고 설계를 바꾼 부분: 처음엔
            change_log 에 개정 이력이 쌓여 있는 줄 알고 "기준일 이후 개정
            건수를 센다"로 잡았는데, 실제로는 1909행이 전부 법당 현재 버전
            하나의 중복이었음(고유 (law_id,new_serial_no) 106개, 법당 1개).
            이력이 없어도 위 논리로 예/아니오는 완전히 답할 수 있어서,
            이력 백필 없이 이 방식으로 감 — 다만 "몇 번, 무엇이" 까지는
            알 수 없으므로 화면에서도 그 이상을 주장하지 않음(자세한
            내용은 /laws/<id> 전문 비교로 넘김).

            DB 조회가 실패했거나 그 법 기록이 없으면 "unknown" — 모르는 것을
            "안 바뀜"으로 뭉개면 안 됨(그게 제일 위험한 오답임).
            """
            if ref_date is None:
                return "n/a"
            if enforce is None:
