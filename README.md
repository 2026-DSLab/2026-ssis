...
- `match_status`: `성공` \| `삭제(위치탐색제외)`(내용 자체가 "<삭제>" 마커라
  ...
  나오고 `unresolved`로 따로 빠진다. `위치재배치의심`이면 `old_text` 맨
  앞에 `[※ 같은 조문 내 항 신설로...]` 안내문이 직접 붙는다(실제 법령
  원문은 대괄호로 시작하지 않으므로 혼동될 일 없음).
...
전체 워치리스트(102건) 기준 실측(2026-07-19, `structural_expansions` 분리
이후 재검증): 74개 그룹, 법 93건, 조문변경(articles) 877건, 구조확장그룹
24건(추가로 107건이 여기 담김), 미확정 18건, 비교불가 9건 → 약 940KB.
