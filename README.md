...
| 화면 | 하는 일 |
|---|---|
| 개정 요약 | 지난 한 주(월~일)에 시행된 개정을 법령별·조문별로 정리. 최근 5일·2주·1개월 기간 조회 |
...
python -m pytest -q          # 480 passed 나오면 정상
```
...
**첫 실행에서는 감시 대상 대부분이 `개정발생` 으로 잡힘.** 전문을 아직 갖고 있지
않아서이지, 지난주에 그만큼 개정됐다는 뜻이 아님. 웹 화면은 시행일 기준으로
걸러 보여주므로 훨씬 적게 나옴(보통 10건 미만). 배치도 그 경우 안내를 출력함.
...
네 화면 모두 **시행일** 로 기간을 자름. 기간 길이만 다름.

| 화면 | 기간 |
|---|---|
| 주간 리포트 | 지난 달력 주 월요일~일요일 |
| 최근 5일·2주·1개월 | 오늘부터 거슬러 N일 |

`주간 리포트` 는 지난 한 주에 시행된 개정을 보여줌. 월요일 배치가 정리하는
기간과 같음. 주중에 배치를 다시 돌려도 화면의 기간은 안 바뀜.

주 후반에 시행된 개정은 그 주가 끝난 뒤 `주간 리포트` 에 들어옴. 그 전에
확인하려면 `최근 5일` 을 씀 — 그래서 `최근 5일` 이 `주간 리포트` 보다 많을 수
있음. 기간이 서로 어긋나 있어 건수가 서로 포함 관계는 아님.

그런 개정이 있으면 `주간 리포트` 화면이 알려 줌.

```
ℹ 이 주 이후에 시행된 개정이 3건 더 있습니다   최근 5일 보기 →
```

개정을 언제 감지했는지는 기간 판정에 쓰지 않음. 배치를 늦게 돌려도 화면에 뜨는
기간은 안 바뀜.
...
| 웹 화면이 비어 있음 | 지난주에 시행된 개정이 없으면 비는 것이 정상. 배치를 거른 경우 화면에 마지막 배치 날짜가 함께 뜸 |
| 주간 리포트보다 최근 5일이 많음 | 기간이 다른 화면임. 주간 리포트 화면의 안내 줄을 따라가면 됨. 위 「화면이 보여주는 기간」 참고 |
