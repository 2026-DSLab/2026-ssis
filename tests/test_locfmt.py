...
report/builder.py(HWPX)와 agents.py("번호만 이동" 규칙
기반 문장)가 같은 위치 표기 규칙을 공유하게 되면서 이 모듈로 뽑혔음.
"""
...
def test_article_of_returns_whole_string_when_no_article_prefix():
    """이동 전 위치처럼 조 번호 없이 항/호만 오는 값은 그대로 돌려줌
...
def test_format_position_handles_branch_numbered_item():
    """실측(국고금관리법 제10조의2②12의2.): "12의2." 같은
    호가지번호는 "의N"까지 통째로 하나의 조각이어야 함."""
