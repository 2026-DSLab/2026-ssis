...
기존엔 테스트가 하나도 없었음. 실측 사고(국가를 당사자로
하는 계약에 관한 법률 제28조②)를 계기로 추가함: describe_change()가
...
자연스러운 문장 속 숫자 환각은 여전히 못 잡는 사각지대가 있었음 —
그 사각지대를 메우는 숫자 검사(③)를 검증함.
"""
...
def test_verify_passes_when_numbers_match_source():
    """올바른 요약(원문에 있는 숫자만 씀)은 숫자 검사에 걸리지 않음."""
    summary = ArticleSummary(
        unit=_unit(),
...
def test_verify_number_check_ignores_rule_based_summaries():
    ...
    검증 대상이 아님 — 숫자 검사도 예외 없이 건너뛰어야 함."""
    unit = _unit(
...
def test_verify_number_check_ignores_list_marker_dot():
    """실측(영유아보육법 제8조②9.): 새 항목 번호 "9. 영유아
    보육..."의 "."은 소수점이 아니라 목록 구분자인데, 정규식이 "9."까지
    통째로 숫자로 잡았음. 요약 문장 "9번 항목으로 옮겨왔습니다"의 "9"는
    뒤에 "번"이 와서 점 없이 "9"만 잡혀, "9." ≠ "9"로 오탐(가짜 환각
    경보)했음. 목록 번호 뒤의 "."은 무시하고 같은 숫자로 봐야 함."""
    unit = _unit(
