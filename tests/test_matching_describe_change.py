...
이전엔 테스트가 하나도 없었다. 실측 버그(2026-07-31)를 계기로 추가한다:
...
def test_describe_change_keeps_whole_word_when_only_a_digit_differs():
    ...
    "'2' → '3'"만 뽑았다. 단위(일)를 잃은 맨 숫자 조각을 받은 LLM이
    "당사자의 수가 2에서 3으로 변경"처럼 완전히 없는 내용을 지어냈다.
    어절 단위로 바꿔 "20일"→"30일" 전체가 한 조각으로 나와야 한다.
    """
    old = (
...
def test_describe_change_single_word_swap():
    ...
    하나의 토큰이다 — 한국어에 조사 앞에 공백을 두지 않기 때문이다.
    """
...
def test_describe_change_returns_empty_for_wholesale_replacement():
    ...
    빈 목록을 돌려준다 — diff_is_meaningful()이 False를 돌려주는 경로."""
    old = "가나다라마바사아자차"
    new = "카타파하거너더러머버"
...
def test_describe_change_merges_word_that_moved_within_a_short_span():
    """★★ 실측(2026-07-31, 전기사업법 제7조의3①): "미리"라는 단어가
    ...
    같은 비문을 만들었다. 사이에 낀 안 바뀐 어절이 짧으면(3어절 이하)
    하나의 완결된 치환으로 합쳐야 한다.
    """
...
def test_describe_change_does_not_merge_unrelated_changes_far_apart():
    ...
    7개나 된다. 이런 먼 변경까지 하나로 합치면 안 된다 — 조각이 흐려져
    LLM이 다시 맥락을 잃는다."""
    old = (
        "이의신청은 이의신청의 원인이 되는 행위가 있었던 날부터 20일 이내 "
        "또는 그 행위가 있음을 안 날부터 15일 이내에 해당 중앙관서의 "
...
def test_describe_change_dedupes_identical_fragments():
    ...
    "(N곳)"으로 합쳐서 LLM이 중복을 문장 내용으로 착각하지 않게 한다."""
