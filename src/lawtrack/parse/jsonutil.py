...
핵심 문제 (팀 전수검증 실측):
    국가법령정보 API 를 type=JSON 으로 호출하면, 반복 요소가
      - 결과 1건  → dict
      - 결과 2건+ → list
    로 다르게 옴. XML 에서는 항상 반복 요소지만 JSON 변환 과정에서
    단일 항목이 배열로 감싸지지 않기 때문임.

    admrul 실측 분포:
        단일(dict) 21건 (87%)  /  복수(list) 3건 (13%)

    => list 를 전제로 파서를 짜면 87% 케이스에서 터짐.
       모든 반복 요소 접근에 as_list() 를 반드시 통과시킴.

이 규칙은 목록조회뿐 아니라 전문 JSON 내부(조문단위, 항, 호, 목)에도
동일하게 적용됨. 조문이 1개인 법령은 dict 로 옴.
"""
...
def dig(data: Any, *keys: str, default: Any = None) -> Any:
    ...
    중간에 list 를 만나면 첫 요소로 내려감. API 응답이 단일/복수를
    오가는 특성 때문에, 경로 탐색 중에도 방어가 필요함.
    """
    cur = data
    for key in keys:
...
def text_of(value: Any, default: str = "") -> str:
    ...
    감싸인 경우가 섞일 수 있으므로 양쪽을 처리함.
    CDATA 로 인해 앞뒤 공백이 붙어 오는 경우가 많아 strip 함.
    """
    if value is None:
        return default
...
def first_key_of(data: Any, candidates: tuple[str, ...]) -> str | None:
    ...
    법령/행정규칙 JSON 의 루트 키 이름을 아직 실측으로 확정하지 못했음.
    (본 프로젝트의 API 검증은 전부 type=XML 로 수행됨)
    하드코딩하면 예측이 틀렸을 때 전부 막히므로, 후보군을 두고 탐색함.
...
def find_key(data: Any, key: str) -> Any:
    ...
    틀렸을 때 전부 막힘. 대신 트리를 순회해 키 이름으로 찾음.
...
def collect_texts(node: Any, text_keys: tuple[str, ...] = ("content", "text", "#text", "value")) -> list[str]:
    ...
    본문 텍스트만 순서대로 뽑아낼 때 씀. dict 에서 text_keys 중
    하나를 발견하면 그 문자열을 잎으로 채택하고 더 깊이 내려가지 않음
    (중복 수집 방지).
    """
    out: list[str] = []
