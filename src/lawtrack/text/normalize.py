...
이 모듈 하나가 실측으로 확인된 매칭 실패 20건 이상을 해소함.

핵심 사실 (실측 검증됨):
  - 가운뎃점처럼 보이는 문자가 5종이며, 서로 다른 코드포인트임.
  - unicodedata.normalize('NFC') 로도, 더 강한 'NFKC' 로도 통일되지 않음.
  - DB 콜레이션(대소문자·악센트 무시 계열)으로도 동일 취급되지 않음.
  => 커스텀 매핑 테이블이 유일한 해법이며, 반드시 애플리케이션 레이어에서
     INSERT/조회 전에 적용해야 함.

정규화 방향은 ㆍ(U+318D)로 통일함. 국가법령정보 API 응답이 이 문자를
사용하는 것이 실측으로 확인되었기 때문임.
...
#: "관리적․기술적" 에서 실제로 발견되어 추가했음. 빠뜨리면 해당 조문만
#: 조용히 매칭 실패함.
...
def normalize_chars(s: str) -> str:
    ...
    가운뎃점 5종은 NFC/NFKC 로 해결되지 않으므로 별도 매핑함.
    """
    if not s:
        return ""
...
def strip_spaces(s: str) -> str:
    ...
    실측: 띄어쓰기 '한 칸' 차이로 매칭 실패가 발생함.
      - 조달청 협상에 의한 계약 제안서평가 세부기준 (<P> 9개 중 일부)
      - 조달청 내자구매업무 처리규정
      - 개인정보의 기술적ㆍ관리적 보호조치 기준
    부분 공백 정규화(연속공백→1칸)로는 부족하므로 전부 제거 후 비교함.
    """
...
def normalize_name(s: str) -> str:
    ...
    정규화로 해결되지 않음. API 응답의 `법령약칭명` 필드로 교차검증하거나
    수동 매핑 테이블(seed)로 처리해야 함.
    """
    s = normalize_chars(s)
    s = _PREFIX_RE.sub("", s)
    s = strip_spaces(s)
...
def normalize_text(s: str) -> str:
    ...
    normalize_name 과 달리 접미사/접두사 변환을 하지 않음.
    조문 내용 자체를 건드리면 의미가 바뀔 수 있기 때문임.
    """
