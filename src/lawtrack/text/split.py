...
#: 항 기호 ①(U+2460) ~ ⑳(U+2473). 법령은 보통 ⑮ 이내이나 여유를 둠.
CIRCLED = "".join(chr(c) for c in range(0x2460, 0x2474))

#: 호: "1." "12." "1의2." — 줄 시작 또는 공백 뒤에서만 인식함.
#: 주의: "제27조" 의 "27" 같은 조문 참조를 호로 오인하면 안 되므로
#:       뒤에 반드시 마침표가 오는 경우만 잡음.
_ITEM_PAT = r"\d+(?:의\d+)?\."
...
#: 마침표 바로 뒤에 공백 없이 호 번호가 붙는 경우가 있음(전자정부법 제56조의2
#: 실측 확인). 반면 아래 두 경우는 호 경계로 오인하면 안 됨:
#:   - "2.1% 이상" 같은 소수점 표기 (직전 2글자가 "숫자+마침표")
#:   - "12의2." 같은 가지번호 표기의 뒷부분 (직전 2글자가 "숫자+의")
#: 실측 확인: (?<!\d) 단독 차단을 없애봤더니 "12." 같은
#: 두 자리 이상 숫자 자체가 "1" + "2." 로 쪼개지는 회귀가 생겼음(아동복지법
#: "12의2." 실측 케이스, split_all 목 분해 케이스 등 다수 실패). 즉 이
#: 1글자 lookbehind는 다자리 숫자 보호에 필수라서 되돌림. "…100분의
...
#: 만으로 해결 가능한 부분만 고침.
#: 실측 발견(조달청 협상에 의한 계약 제안서평가 세부기준
...
#: 둘로 쪼개지고 old_text가 양쪽에 중복 삽입되는 결과를 냈음. "장/절/관/편"
#: 은 법령 문서구조 표준 단위(편>장>절>관>조)라 "○의" 바로 뒤에 숫자+마침표가
#: 오면 거의 항상 이런 외부참조이지 이 조문의 실제 호 목록 시작이 아님.
_SECTION_UNITS = "장절관편"
...
#: 한글 문장 종결형("포함한다.", "같다." 등)의 마지막 글자와 우연히 겹침.
#: 반대로 실측(전자정부법 제2조제11호): 실제 목 마커도 앞 목의 내용에
#: 공백 없이 바로 붙는 경우가 있음("…행정정보나. 정보시스템다. …", 즉
#: "나." 바로 앞이 한글 음절 "보"). 그래서 앞 글자가 한글이냐 아니냐로는
#: 참/거짓을 구분할 수 없음 — 대신 뒤쪽에서 실제로 "가나다라…" 순서로
#: 증가하는 마커 나열이 만들어지는지(_valid_subitem_run)로 판정함.
...
def split_by_subitem(text: str) -> list[Fragment]:
    ...
    바로 붙기도 함("…행정정보나. 정보시스템다. …"). 반면 한글 문장
    종결형("포함한다.")도 우연히 목 마커와 같은 글자를 갖음. 둘 다
    "바로 앞이 한글이냐"로는 구분이 안 되므로, 대신 뒤쪽에서 실제로
    "가→나→다→…" 순서로 엄격히 증가하는 마커 나열이 나오는지 검증함.
    그런 나열이 없으면(예: 우연히 하나 걸린 "다.") 분해하지 않음.
...
def _mask_parens_digits(text: str) -> str:
    """괄호 안의 숫자만 자릿수를 유지한 채 '#'로 가린 텍스트를 만듦.

    반환값은 오직 "경계가 어디인지" 찾는 용도로만 쓰고, 실제 조각의
    내용(Fragment.text/raw)은 항상 원본 text에서 그대로 잘라 씀 —
    길이를 그대로 유지해야(숫자 1글자 → '#' 1글자) 원본 text와 위치가
    1:1로 대응해 슬라이싱이 어긋나지 않음.
    """
...
def _split(
    ...
    if mask_parens:
        ...
        # 마스킹된 텍스트로 하되 실제 자르기는 원본 text에서 함
        # (마스킹은 길이를 보존하므로 위치가 그대로 대응됨).
...
def split_all(text: str) -> list[Fragment]:
    ...
    검색 시에는 가장 작은 단위부터 시도하는 것이 매칭 확률이 높음.
...
def leading_marker(text: str) -> Fragment | None:
    """텍스트 맨 앞에 있는 항/호/목 기호 하나만 가볍게 뽑음(전체 분해 없이).

    쓰임(삭제 항목 위치 표시): 삭제(DELETED)된 항목은 신법에
    더 이상 존재하지 않아 locate 6가드가 위치 탐색을 아예 생략함(조 번호를
    ...
    알 수 있음 — 재조회 없이 이미 가진 데이터에서 뽑아내는 것.
    """
...
#: 실측 발견(보안업무규정 시행규칙 제56조①): "3.·4. (생 략)"
#: 처럼 "∼" 대신 가운뎃점(U+00B7 MIDDLE DOT)으로 인접한 두 호를 이은
#: 표기도 있음("1.·2." 도 같은 조문 ③에서 실측됨). 지금까지 관측된 건
#: 모두 바로 이웃한 두 번호(3·4, 1·2)뿐이라 범위로 펼쳐도 나열로 읽어도
#: 결과가 같음 — "·"가 언젠가 안 이웃한 번호를 잇는 경우가 나오면(예:
#: "2.·5.") 이 가정이 깨지므로 그때 다시 봐야 함.
...
def parse_skip_range(text: str) -> list[str] | None:
    """스킵 표시 조각에서 "안 바뀐" 항/호 라벨 목록을 뽑음.

    설계(admrul-only unchanged 감지): 행정규칙은 법령과
    달리 항제개정유형 같은 공식 "안 바뀜" 태그가 없음. 대신 신구법
    ...
    왔음(extract_changes). 이 함수는 그 표시를 실제 라벨 목록으로
    되살림. 스킵 표시가 아니면 None.
    """
...
#: 벗겨지므로 여기 도달하는 시점엔 절대 <P>일 수 없음.
_ANNOTATION_RE = re.compile(r"<[^<>]*>")

#: 실측 발견((계약예규) 협상에 의한 계약체결기준): oldAndNew 블록 경계에서
#: 각주가 "<개정 2020.9.24." 처럼 닫는 ">" 없이 잘리는 경우가 있음(다음
...
#: 각주도 있음 — parse/fulltext.py 가 이미 알고 있는 "조문참고자료" 필드와
#: 같은 종류의 메타데이터가 oldAndNew 평문 블록에는 그대로 섞여 나옴.
...
def strip_annotations(text: str) -> str:
    ...
    실측 발견((계약예규) 예정가격작성기준): "<개정
    2011.5.13., 2015.9.21., 2025.5.1.>" 처럼 조문 문장 중간에 개정이력
    각주가 그대로 섞여 있는 경우가 있음. 이 안의 "5.13." "9.21." 같은
    날짜 조각이 호 번호 패턴(\\d+\\.)과 우연히 겹쳐, 목 분해와 똑같은
    문제(가짜 경계로 오분해)를 일으킴 — 실제 현행 조문에는 이 각주
    자체가 없으므로 애초에 검색 대상에서 제거하는 게 근본 해법임.
    "<img id="...">" 같은 이미지 태그도 같은 이유로 함께 제거됨.
    classify()가 이미 <신 설>/<삭제> 같은 전체-블록 마커를 판정에 쓴
    뒤이므로, 이 함수는 그 판정이 끝난 '검색용' 텍스트에만 적용함
    (locate 단계 진입 직전).
    """
...
def strip_article_head(text: str) -> str:
    """맨 앞의 '제N조(의M)(제목)' 헤더를 제거하고 본문만 남김.

    실측 발견: oldAndNew 블록이 "제6조의2(기준 중위소득의 산정) ① 기준
    중위소득은 …" 처럼 조문 제목으로 시작하는 경우가 있음. 이 제목
    부분은 실제 조문 '내용'이 아니라 헤더이므로, lawService 항내용
    필드에는 보통 포함되지 않음. 그대로 두면 항상 0건 매칭되는
    무의미한 조각이 하나 더 생기므로, 위치 검색 전에 제거함.
    """
    text = (text or "").strip()
