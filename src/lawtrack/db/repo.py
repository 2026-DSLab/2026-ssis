...
이 파일이 SQL 을 아는 유일한 곳임. 파이프라인 코드는 SQL 을 직접
쓰지 않고 이 레포지토리들을 통해서만 DB 를 만짐.
"""
...
def _parse_iso_date(value: str | date | None) -> date | None:
    ...
    계약 JSON(contract/schema.py)은 날짜를 문자열로 실어 나름 — 요약 단계에
    넘기는 JSON 이라 타입이 아니라 표기가 계약이기 때문임. 그 값이 DB로
    돌아올 때(요약 적재) 여기서 다시 date 로 바꿈. 빈 문자열도 정상
    입력임(시행일 미상) — 그때는 NULL 로 들어감.
    """
...
class WatchlistRepo:
    ...
    def update_last_seen(self, law_id: str, serial_no: str, *, checked_at: datetime | None = None) -> None:
        ...
        이걸 빠뜨리면 다음 주에 같은 개정을 또 감지해 중복 보고함.
        """
        with self._db.transaction() as (_, cur):
            cur.execute(
...
class VersionRepo:
    ...
    설계: laws/administrative_rules 두 테이블을
    documents 하나로 합쳤음. 컬럼 구성이 이름만 다를 뿐 완전히 같았고
    (이름/ID/일련번호/전문JSON/타임스탬프), repo.py에 거의 동일한 CRUD
    코드가 두 벌 있었음 — kind 구분 컬럼('law'/'admrul') 하나로 그 중복을
    없앰. watchlist.law_type("법률"/"시행령"/"행정규칙" 등 세부 종류)과
    헷갈리지 않게, kind는 그 둘만 구분하는 내부 전용 값으로 별도로 둠.
    """
    ...
    def law_exists(self, law_id: str, serial_no: str) -> bool:
        """SELECT 1 로 존재 여부만 확인 — 이게 곧 '개정 감지' 판정임."""
    ...
    def insert_law(self, law_name: str, law_id: str, serial_no: str, full_text: dict) -> None:
        ...
        배치에서는 행 자체가 없을 수 있으므로 INSERT 를 씀.

        full_text는 절대 가공하지 않고 그대로 보존함 — 파서를 고치는 일이
        잦기 때문에, 원본이 남아있어야 고친 뒤 재처리해서 검증할 수 있음.
        (조/항/호/목으로 미리 파싱한
        캐시 컬럼은 두지 않음 — 아무 코드도 그
        캐시를 다시 읽지 않았고, 필요하면 parse_articles(full_text)로
        언제든 그 자리에서 다시 만들 수 있음.)
        """
    ...
    def insert_admrul(self, rule_name: str, rule_id: str, serial_no: str, full_text: dict) -> None:
        """insert_law() 와 동일한 취지 — full_text(원본)만 보존함."""
...
class ChangeLogRepo:
    ...
    def fetch_latest_for_serial(self, law_id: str, new_serial_no: str) -> dict | None:
        ...
        실측 발견(contract/export.py 실데이터 검증): 이 조회가
        ...
        연쇄개정 그룹핑(link.py)이 절대 작동하지 않는 버그가 있었음
...
#: 설계: match_status="위치재배치의심"일 때 old_text 앞에
#: "[※...]" 안내문을 붙이던 로직은 제거했음 — match_status 필드 자체가
#: 이미 "위치재배치의심"이라는 값으로 신뢰도를 명시하므로, old_text에
#: 텍스트를 덧붙이면 DB에 저장되는 old_text가 원문 그대로가 아니게 되는
#: 부작용이 있었음. DB에 저장되는 old_text는 항상 순수 원문이어야 함.
...
class ArticleDiffRepo:
    ...
    def insert_results(
        ...
        *, default_enforce_date: date,
        ...
        for change, locate_results in results:
            ...
            # 폴백함(all-or-nothing — 일부만 정밀 매칭하는 애매한 상태는
            # 만들지 않음).
        ...
        # 실측 발견(행정규칙 재처리 검증): 실패 건의 key가
        # __unresolved__{change.index}_{frag_idx} 처럼 "이번 계산 결과"에
        # 따라 달라질 수 있음(예: 예전엔 실패해서 unresolved 키로 들어갔다가,
        ...
        # 남음 — 재처리할 때마다 실패 기록이 누적되는 버그였음. 이번 계산
        # 결과가 그 (law_id, law_serial_no)의 유일한 진실이므로, 매번 먼저
        # 완전히 비우고 다시 채움(같은 트랜잭션 안이라 원자적).
        # 실측 발견((계약예규) 정부 입찰ㆍ계약 집행기준 34470): PostgreSQL 의
        # ON CONFLICT DO UPDATE 는 한 INSERT 문 안에서 같은 행을 두 번 건드리는
        # 것을 허용하지 않고 CardinalityViolation 으로 배치 전체를 실패시킴.
        # execute_values 로 여러 행을 한 문장에 담기 때문에 이 제약에 걸림.
        #
        # 실제로 results 에는 change.index 가 같은 ArticleChange 가 두 번 이상
        # 섞여 들어오는 경우가 있음(위치확정 실패 건의 합성키
        # __unresolved__{index}_{frag_idx} 가 그래서 충돌). 상위 파이프라인에서
        # 걸러야 할 값일 수도 있지만, DB 계층은 "마지막 값이 이긴다"는 관용성을
        # 유지함 — 그래야 한 건 때문에 배치 전체가 죽지 않음.
        # 같은 UNIQUE KEY를 가진 행을 미리 걸러 마지막 값만 남김.
    ...
    def fetch_period(self, from_date: date, to_date: date) -> list[dict]:
        """계약 산출물(contract) 조립 시 사용할 조회.

        기간은 감지일(created_at) 기준이 아니라 시행일(enforce_date)
        기준으로 둘 다 열어 둠 — 호출부가 목적에 맞게 고름.
        """
        with self._db.cursor() as (_, cur):
            cur.execute(
    ...
    def _fragment_old_text_by_item(
        ...
        없이 1:1 매칭할 수 있으면 그 매핑을 돌려줌 — 대응이 하나라도
        안 되거나 old쪽에 같은 키가 중복되면(애매함) None을 돌려줘
        호출부가 기존 통짜-재사용 동작으로 폴백하게 함. all-or-nothing:
        일부 위치만 정밀 매칭되는 상태는 만들지 않음(그 change의 여러
    ...
    def _reshuffled_articles(
        ...
        등) 일반 규칙으로 재정렬할 수 없음. 그렇다고 "성공"으로 조용히
        내보내면 서로 무관한 구/신 문장을 마치 같은 조항의 전후인 것처럼
        요약 단계에 확정 사실로 전달하게 됨. 내용 기반 재정렬은 시도하지
        않고(오탐 위험이 더 크다는 사용자 판단), 대신 순수 신설이 섞인
        조문 안의 '개정' 항목은 전부 의심 대상으로 표시만 함(over-flag
    ...
    def _to_row(
        ...
        # 조문 단위 시행일이 없으면(빈 문자열 등) 개정 전체 시행일로 보강함.
        ...
        if unit:
            ...
            # 실측 발견: 위치확정 실패(0건실패/중복실패)나
            # 삭제 스킵(DELETED_SKIP)은 unit이 없어 article_code 등이
            # 전부 빈 문자열이 됨. UNIQUE KEY가 (law_id, law_serial_no,
            ...
            # 처리된 삭제 건이 실패한 것처럼 보였음. change_type으로
            # 구분해 삭제는 명확한 라벨을 씀 — article_code(DB 유일성
            # 목적)는 그대로 두고 article_label(사람이 보는 값)만 바꿈.
            if change.change_type is ChangeType.DELETED:
                # 실측 발견(지능정보화 기본법 후속
                # 사용자 질문): "삭제됨"이라고만 하면 원래 몇 조 몇 항/호/목
                # 이던 자리인지 안 보임. old_text 조각 자체는 보통
                # "① …"처럼 항/호/목 기호로 시작하므로 그 선행 기호는
                # 재조회 없이 이미 가진 데이터에서 뽑아낼 수 있고
                # (leading_marker), 조 번호는 change.article_context가
                # 채워줌 — extract_changes()가 old_texts 전체를 순서대로
                # 훑어 "지금 어느 조문 안인가"를 추적한 결과임(같은 블록에
        ...
        # 실측 발견(contract/export.py 실데이터 검증): 한 change가
        # 여러 조각(항①②③…)으로 쪼개지면(6가드 가드④), 그동안 모든 조각의
        # new_text 에 change.new_clean(쪼개지기 전 원본 통짜 블록)을 그대로
        # 써왔음 — 그 결과 제56조의2 항①~⑤ 등 서로 다른 위치의 행들이 전부
        # 똑같이 거대한 원문 덩어리를 new_text 로 갖게 되어, "이 위치에서
        # 정확히 무엇이 바뀌었는지"를 각 행만 보고는 알 수 없었다(요약 단계에
        # "이미 확정된 사실만 준다"는 설계 원칙 위반). new_text 는 실제로
        # 위치가 확정된 조각(lr.fragment)의 텍스트를 씀.
        #
        # 실측 발견((계약예규) 예정가격작성기준 제40조② 등):
        ...
        # 그대로 써서 이런 태그가 요약 단계에 그대로 노출되고 있었음. 같은
        # 함수를 old_text 에도 적용함. "<신  설>" 처럼 각주 자체가 전체
        # 내용인 경우는 strip 후 공백만 남는데, change_type 필드가 이미
        # "신설"을 명시하므로 정보 손실이 아님(오히려 "old_text=<신설>"
        # 이라는 내부 마커 텍스트를 그대로 노출하는 것보다 빈 문자열이
        # "개정 전엔 없었다"는 뜻을 더 명확히 전달함).
        #
        # 설계: old_text도 fragment_old_lookup이 있으면
        # (호 단위까지 구/신이 정밀하게 1:1 대응된 경우) 통짜 change.old_clean
        # 대신 이 위치에 해당하는 호 단위 조각만 씀 — insert_results()가
...
class LawSummaryRepo:
    ...
    왜 summarizer 의 자료구조(LawSummary)를 인자로 받지 않고 원시값만
      받는가: 의존 방향을 지키기 위해서임. summarizer 는 lawtrack 을
      import 하지만(계약 스키마를 읽어야 하므로) 그 반대는 아님.
      여기서 LawSummary 를 import 하면 순환이 생기고, 감지 파이프라인만
      쓰려는 사람도 LLM 쪽 의존성을 깔아야 함. 변환은 부르는 쪽
      (summarizer/sinks.py 의 DbSink)이 함.
    """
    ...
    def upsert(
        ...
        """요약 1건 저장. 같은 (law_id, new_serial_no) 가 있으면 덮어씀.

        덮어쓰는 이유는 law_summary 테이블 COMMENT 에 적어 두었음 — 요약은
        계약 JSON 에서 언제든 다시 만들 수 있는 파생물이고, 실제로 참조되는
        것은 항상 최신 1건임.

        new_serial_no 가 비어 있으면 저장하지 않고 예외를 냄. 빈 문자열로
          넣으면 서로 다른 개정분의 요약이 같은 키('')로 몰려 마지막 것만
          남음 — 조용히 데이터가 사라지는 종류의 버그라 여기서 막음.
        """
    ...
    def fetch(self, law_id: str, new_serial_no: str) -> dict | None:
        """요약 1건 조회. JSON 컬럼은 파이썬 객체로 풀어서 돌려줌."""
        with self._db.cursor() as (_, cur):
            cur.execute(
