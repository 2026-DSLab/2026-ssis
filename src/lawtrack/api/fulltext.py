...
빈 응답이 오는데, 겉보기엔 '호출은 성공'했기 때문에 원인 파악이 늦어짐.
그래서 이 모듈에서 파라미터 분기를 강제하고, 호출부는 law_id 계열인지
admrul_id 계열인지만 신경 쓰면 되게 함.
"""
...
class FullTextResult:
    ...
    JSON 컬럼에 저장할 수 있게 함. 파싱된 필드는 파이프라인 판단용."""
...
def fetch_admrul_fulltext(client: LawApiClient, rule_serial_no: str) -> FullTextResult:
    ...
    주의: 여기서의 ID 는 행정규칙ID 가 아니라 행정규칙일련번호임.
    목록조회 응답의 상세링크가 이를 실측으로 확인해줌.
...
def _build_admrul_result(data: dict, serial_no: str) -> FullTextResult:
    # 실측 확인(여러 ID 교차검증): 실제 루트 키는 "AdmRulService"
    # 뿐임. "행정규칙"/"AdmRul"은 확인된 적 없는 추측 후보였고, 매번 이
    # 뒤의 세 번째 후보(AdmRulService)로만 성공해왔음. 실측 키를 앞에 두어
    # 불필요한 순회를 없앰.
    ...
    # 있으면 채우고 없으면 빈 문자열로 둠 (assert 하지 않음).
    reason = _extract_nested_text(root, "제개정이유", "제개정이유내용")
    revision_text = _extract_nested_text(root, "개정문", "개정문내용")
...
def _extract_nested_text(root: dict, outer_key: str, inner_key: str) -> str:
    """"제개정이유"/"개정문" 처럼 한 겹 더 안에 실제 내용이 있는 필드를 읽음.

    실측 발견(공공기관의 정보공개에 관한 법률 등): root의
    "제개정이유"/"개정문" 키는 그 자체가 문자열이 아니라
    {"제개정이유내용": [[...줄들...]]} / {"개정문내용": [[...줄들...]]}
    형태로 한 겹 더 감싸여 있음. 예전엔 _dig_any(root, ("제개정이유",
    "제개정이유내용"))가 이 둘을 "같은 레벨의 후보"로 취급해, 실제로는
    항상 "제개정이유"(딕셔너리)가 먼저 걸려 그 딕셔너리 자체를
    text_of()에 넘겼는데, text_of()는 dict에서 "#text"/"content"/
    "value"/"_" 키만 볼 뿐이라 늘 빈 문자열로 떨어졌음 — schema.py 가
    강조하는 revision_reason 필드가 이 때문에 한 번도 채워진 적이 없었음.
