...
def get_or_fetch_full_text(
    ...
    부를 필요가 없다.
    """
    # ★ 실측 버그(2026-08-18, 전수검증 — 전문 비교에서 "전문 내용을 찾지
    #   못했습니다"만 뜨는 법 2건): documents 에 full_text 가 {} 인 빈
    #   껍데기 행이 있었다(국민기초생활보장법 281585, 조달청 내자구매업무
    #   처리규정 2100000280340 — 둘 다 2026-07-15 22:14 저장으로, 파이프라인
    #   첫 실행(7-16)보다 앞선 초기 세팅 때 들어간 자리표시자다).
    #   "행이 있으면 캐시 적중"으로만 보면 이 빈 값을 정상 캐시로 믿고
    #   그대로 돌려줘, 파서가 유닛 0개를 내고 화면이 빈 채로 남는다.
    #   내용이 없는 캐시는 캐시가 아니므로 미적중으로 보고 다시 받아온다
    #   (_insert 가 upsert 라 받아오면 그 자리에서 빈 행이 덮어써진다).
...
def build_version_chain(
    ...
    chain = []
    for s in serials:
        # ★ 실측 버그(2026-08-18, 전수검증 — (계약예규) 정부 입찰·계약
        #   집행기준에서 HTTP 500): 과거 버전 하나의 전문을 API가 비정상
        #   응답(46자짜리 껍데기)으로 돌려주면 LawApiFormatError 가 그대로
        #   올라와 페이지 전체가 죽었다. 한 버전을 못 받은 것과 "이 법은
        #   아예 못 본다"는 전혀 다른 얘기다 — 받아온 버전만으로도 비교는
        #   보여줄 수 있으므로, 실패한 버전은 체인에서 빼고 계속한다.
        #   전부 실패하면 빈 체인이 되고, 호출부가 그때 안내를 띄운다.
        try:
