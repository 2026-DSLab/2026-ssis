...
이 파일이 감지·비교 단계와 요약 단계 사이의 '계약'임. Pydantic 모델로 정의해
내보내기 직전에 자기 검증을 거치게 함 — 필드 하나가 빠진 채로
넘어가면 요약 단계에서 원인 모를 오류가 나므로, 여기서 막음.

설계 원칙 (프로젝트 전체에서 지켜온 것):
    LLM 에게는 이미 확정된 사실만 줌. "무엇이 바뀌었는지 찾아라"는
    시키지 않음 — 그건 <P> 태그와 6가드가 이미 결정론적으로 확정한
    것임. LLM 의 역할은 이 사실들을 문장으로 다듬는 것뿐임.

구조가 amendment_groups(공포번호 그룹) 를 최상위로 두는 이유:
    실측: 공포번호 하나로 5개+ 법이 동시 개정됨. 법을 평평하게
    나열하면 LLM 이 "정부조직 개편 일괄정비" 같은 맥락을 못 잡고
    무관해 보이는 항목들을 따로따로 서술함.

unresolved 를 절대 비우지 않는 이유:
    6가드로 위치 확정에 실패한 것을 조용히 빼면, LLM 이 "확정 안 됨"을
    모른 채 문장을 만들 위험(환각)이 생김. 실패는 실패로 명시함.
"""
...
class UnresolvedItem(BaseModel):
    """6가드로 위치 확정에 실패한 건. 절대 조용히 빠지지 않음."""
...
class WeeklyContract(BaseModel):
    """주간 산출물 최상위 스키마. 요약 단계가 받는 실제 구조."""
