...
주의 — API 키 노출 방지:
    ...
    쿼리스트링에 그대로 박혀 있음(형태: '/DRF/lawService.do?OC=<OC인증키>
    &target=law&MST=...'). 이걸 그대로 산출물에 옮기면 인증키가
    계약 산출물(파일)에 새어나감. source_url 생성 시 반드시 OC 를
    제거하거나 안전한 값으로 치환함.

    이 설명에도 한때 실측한 URL 을 그대로 붙여 놔서 진짜 OC 값이 이
    독스트링에 박혀 있었음 — 배포 꾸러미 검사에서 발견해 자리표시자로
    바꿨음. 예시를 적을 때 실제 응답을 그대로 붙이지 말 것.
"""
